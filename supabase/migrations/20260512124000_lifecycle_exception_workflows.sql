-- AWO-81: named-admin lifecycle exception transitions.
-- This is the protected foundation for refund/revocation/transfer operations.
-- It does not call Stripe or move live money.
...
begin
  ...
  where order_item_id = item_record.id
  for update;
  ...
  if normalized_action = 'revoke_credential' then
    ...
    set
      ...
      updated_at = now()
    where order_item_id = item_record.id
    returning credential_status into next_credential_status;
