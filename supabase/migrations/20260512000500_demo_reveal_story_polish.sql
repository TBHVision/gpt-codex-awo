-- AWO-76: make the seeded honoree reveal read like a stakeholder demo.
-- This keeps the reveal safety checks from AWO-47, then enriches the
-- successful playback payload with concrete provenance and ownership context.
...
begin
  ...
  select
    ...
    jsonb_build_array(
      ...
      jsonb_build_object(
        'label', 'Gifted',
        'value', 'Recipient path prepared for ' || coalesce(reveal_record.recipient_name, 'the recipient') || '.'
      ),
      jsonb_build_object(
        'label', 'Revealed',
    ...
    (
      ...
      coalesce(reveal_record.card_title, 'this AWO card') || '. ' ||
