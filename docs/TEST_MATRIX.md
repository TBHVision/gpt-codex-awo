# Test Matrix

| Version | Codex Tests | Human Tests |
| --- | --- | --- |
| V0.0 | build, lint, dashboard route, responsive layout, docs present | dashboard clarity, phase model approval, next-action clarity |
| V0.1 | admin gate redirect, login route, logout route, migrations, RLS policy catalog checks, auth session, admin bootstrap, seed data | login flow, admin access model, environment setup |
| V0.2 | shop render, filters, card detail, cart persistence, cart/checkout edit controls, checkout steps | browse feel, card trust, checkout clarity |
| V0.3 | QR/PIN Supabase RPC, invalid PIN state, successful reveal payload, mobile viewport | emotional reveal, recipient clarity, artist trust |
| V0.4 | people CRUD, occasion CRUD, calendar/feed, find-card link | warm non-CRM feel, reminder usefulness |
| V0.5 | public artist profiles, Supabase studio drafts, provenance checklist persistence, local fallback | artist confidence, submission clarity |
| V0.6 | user approvals, card approvals, orders, fraud logs, analytics | ops usability, admin trust |
| V0.7 | release gate, visual QA, deployed freshness guard, authenticated E2E, observability posture | release confidence |
| V1.0 | full smoke suite, production build, env audit, security checks | launch review |
